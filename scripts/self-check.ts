...
async function main() {
  await check("detectSignals finds prose signals, skips code lines", () => {
    ...
    assert.deepStrictEqual(detectSignals("// error: handled\n{ failed: true }"), []);
    assert.deepStrictEqual(detectSignals(""), []);
  });

  await check("filterRelevant keeps recent high-score successes, caps at 3", () => {
    const now = Date.now();
    const iso = (msAgo: number) => new Date(now - msAgo).toISOString();
    const mk = (score: number, status: string, msAgo: number): OutcomeEntry => ({
      timestamp: iso(msAgo),
  ...
  await check("recordOutcome classifies a real git diff into the graph", async () => {
    ...
    assert.strictEqual(after, before, "dedupe must suppress the second record");
    delete process.env.MEMORY_GRAPH_PATH;
  });
